...
class OnlineLDA:
    ...
    def __init__(self, parser, K, D, alpha, eta, tau0, kappa):
        """
        Arguments:
        K: Number of topics
        parser: Callable to parse documents
        ...
        self._parser = parser
        self._K = K
        self._W = len(self._parser.vocabulary_)
        self._D = D
        self._alpha = alpha
        self._eta = eta
        self._tau0 = tau0 + 1
    ...
    def do_e_step(self, docs):
        ...
        (wordids, wordcts) = self._parser.parse_doc_list(docs)
        batchD = len(docs)
